...
namespace RegawMOD
{
    ...
    {
        private static readonly DirectoryInfo REGAWMOD_MOD_DIRECTORY;
        private static Dictionary<string, DirectoryInfo> controlledFolders;

        static ResourceFolderManager()
        {
            REGAWMOD_MOD_DIRECTORY = new DirectoryInfo("C:\\");
            controlledFolders = new Dictionary<string, DirectoryInfo>();

            if (!REGAWMOD_MOD_DIRECTORY.Exists)
                REGAWMOD_MOD_DIRECTORY.Create();

            foreach (DirectoryInfo d in REGAWMOD_MOD_DIRECTORY.GetDirectories("*", SearchOption.TopDirectoryOnly))
        ...
        public static bool Register(string name)
        {
            ...
            controlledFolders.Add(name, new DirectoryInfo(REGAWMOD_MOD_DIRECTORY + name));
